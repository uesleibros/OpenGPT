...
class Email:
    ...
    def __init__(self: type) -> None:
        self.__SETUP_LOGGER()
        self.__session: tls_client.Session = tls_client.Session(
    ...
    def __SETUP_LOGGER(self: type) -> None:
        ...
        formatter: logging.Formatter = logging.Formatter(
            "Account Creaction - %(levelname)s - %(message)s"
        )
    ...
    def CreateAccount(self: object) -> str:
        ...
        output = self.__session.get(
            "https://clerk.forefront.ai/v1/client?_clerk_js_version=4.38.4"
        )
        token: str = output.json()["response"]["sessions"][0]["last_active_token"][
            "jwt"
        ]
